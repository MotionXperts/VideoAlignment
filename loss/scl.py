...
class SCL(object):
    ...
    def ori_compute_loss(self, model, videos, seq_lens, chosen_steps, video_masks=None, training=True,DEBUG=False,images=None,summary_writer=None,epoch=None,split="train"):
        ...
        # embs = model(videos, video_mask=video_masks, split=split)
        embs = model(videos, video_masks=video_masks, project=split=="train")

        embs = embs.view(batch_size, num_views, num_frames, embs.size(-1))

        seq_lens = seq_lens.view(batch_size, num_views)
        loss = self.compute_loss(embs, seq_lens.to(embs.device), chosen_steps.to(embs.device), video_masks.to(embs.device))
        ic(loss)
        return loss
