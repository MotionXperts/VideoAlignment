...
class Pouring(torch.utils.data.Dataset):
    def __init__(self, cfg, split, mode="auto", sample_all=False):
        ...
        if not self.sample_all:
            ...
            ## print the name of the longest video
            video_idx= np.argmax(seq_lens)
            hist, bins = np.histogram(seq_lens, bins='auto')
            print("|====== The distribution of video length is =======|")
            print("|The longest video is: "  , self.dataset[video_idx]['name'], "with the length: " ,  seq_lens[video_idx])
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.SSL and not self.sample_all:
            ...
            # aug_0 = self.b4_norm(video[steps_0.long()])
            # aug_1 = self.b4_norm(video[steps_1.long()])
            # if self.printed < 10 and du.is_root_proc():
            #     self.visualize_augemented_video(video,aug_0,aug_1)
            #     self.printed +=1
    ...
    def visualize_augemented_video(self,video,aug:torch.tensor,aug1):
        """
        Visualize the augmented view.
        ------
        Input:
            aug: (T, H, W, C) tensor
        Output:
            No output. Directly write into test.mp4
        """
        import os
        from torchvision.io import write_video
        video = video.permute(0,2,3,1) * 255.0
        aug = aug.permute(0,2,3,1) * 255.0
        aug1 = aug1.permute(0,2,3,1) * 255.0
        # print(video.shape,video[0])
        # print(aug.shape,aug[0])
        os.makedirs('aug_videos_vis',exist_ok=True)
        write_video(os.path.join('aug_videos_vis',f'video_{self.printed}.mp4'), video, 60)
        write_video(os.path.join('aug_videos_vis',f'test_{self.printed}_0.mp4'), aug, 60)
        write_video(os.path.join('aug_videos_vis',f'test_{self.printed}_1.mp4'), aug1, 60)
